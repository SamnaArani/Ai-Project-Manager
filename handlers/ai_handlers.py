...
async def check_ai_access(user_id: str, request_type: str) -> Tuple[bool, str, dict, dict]:
    ...
    # --- BUG FIX: Handle NoneType for last_usage_date ---
...
async def _execute_tool_and_handle_response(tool_name: str, args: dict, update: Update, context: ContextTypes.DEFAULT_TYPE, placeholder_message_id: int):
    ...
    try:
        ...
        # --- BUG FIX: Create a copy of args to avoid mutating the loggable `plan` object ---
        executable_args = args.copy()
        
        sig = inspect.signature(tool_function)
        if 'update' in sig.parameters: executable_args['update'] = update
        if 'context' in sig.parameters: executable_args['context'] = context
        
        result = await tool_function(**executable_args)
