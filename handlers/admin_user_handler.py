...
async def view_user_details(update: Update, context: ContextTypes.DEFAULT_TYPE, user_telegram_id: str):
    ...
    daily_chat = user_doc.get('daily_chat_usage', 0)
    monthly_chat = user_doc.get('monthly_chat_usage', 0)
    daily_cmd = user_doc.get('daily_command_usage', 0)
    monthly_cmd = user_doc.get('monthly_command_usage', 0)
    ai_usage_text = f"چت روزانه: {daily_chat} | چت ماهانه: {monthly_chat}\n" \
                    f"دستور روزانه: {daily_cmd} | دستور ماهانه: {monthly_cmd}"
    ...
    text = (f"👤 *مشخصات کاربر: {escaped_display_name} {admin_marker}*\n\n"
            ...
            f"🗓️ *تاریخ فعال‌سازی:* {common.format_datetime_field(user_doc.get('package_activation_date'))}\n"
            f"⏳ *تاریخ انقضا:* {common.format_datetime_field(user_doc.get('package_expiry_date'))}\n\n"
            f"🤖 *مصرف هوش مصنوعی:*\n{ai_usage_text}")
