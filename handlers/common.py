...
import logging
import re
from telegram import Update, InlineKeyboardMarkup, ReplyKeyboardMarkup, KeyboardButton, CallbackQuery
...
async def send_or_edit(update_or_query: Update | CallbackQuery, text: str, reply_markup: InlineKeyboardMarkup = None, parse_mode='Markdown'):
    """
    ...
    if not target:
        logger.warning("send_or_edit could not find a target message to send or edit.")
        return

    try:
        if is_callback:
            # Avoid sending an edit request if the content is identical
            if target.text != text or target.reply_markup != reply_markup:
        ...
        if "Message is not modified" not in str(e):
            logger.warning(f"Could not edit message, trying to send new one. Error: {e}")
            try:
                # Fallback to sending a new message if editing fails for other reasons
                await target.chat.send_message(text, reply_markup=reply_markup, parse_mode=parse_mode)
