...
# --- Helper Function ---
async def check_and_reprompt_for_token(update: Update, context: ContextTypes.DEFAULT_TYPE, custom_text: str):
    """Checks if the user is stuck waiting for a token and reprompts them after a support interaction."""
    user_id = str(update.effective_user.id)
    user_doc = await asyncio.to_thread(
        database.get_single_document, config.APPWRITE_DATABASE_ID, config.BOT_USERS_COLLECTION_ID, 'telegram_id', user_id
    )
    
    # Re-prompt only if the user has started registration but hasn't provided a valid token yet
    if user_doc and not user_doc.get('clickup_token'):
        await common.show_limited_menu(update, custom_text)
...
async def start_support_conversation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Set a flag to indicate the user is in the support flow
    context.chat_data['in_support_flow'] = True
...
async def user_message_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Unset the support flow flag
    context.chat_data.pop('in_support_flow', None)
    ...
    for admin in admins:
        try:
            ...
            # Update the admin panel to show the new message count
    ...
    # After finishing, check if the user needs to be re-prompted for a token
    await check_and_reprompt_for_token(update, context, 
        "پیام شما به پشتیبانی ارسال شد. برای ادامه فرآیند ثبت نام، لطفاً توکن ClickUp خود را وارد کنید."
    )
...
async def cancel_user_support(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Cancels the user support conversation and reprompts if needed."""
    # Unset the support flow flag
    context.chat_data.pop('in_support_flow', None)
    
    await common.generic_cancel_conversation(update, context)
    
    # After cancelling, check if the user needs to be re-prompted for a token
    await check_and_reprompt_for_token(update, context,
        "عملیات پشتیبانی لغو شد. برای ادامه فرآیند ثبت نام، لطفاً توکن ClickUp خود را وارد کنید."
    )
    return ConversationHandler.END
...
async def manage_messages_entry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not all_tickets:
        ...
        sorted_users = sorted(user_conversations.items(), key=lambda item: item[1]['user_info']['created_at'], reverse=True)
...
async def admin_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif action == "back" and data_parts[3] == "inbox":
